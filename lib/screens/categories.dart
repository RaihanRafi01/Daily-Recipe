...
class _CategoriesScreenState extends State<CategoriesScreen> with  SingleTickerProviderStateMixin{
  ...
  void _selectCategory(BuildContext context, Category category){
    final filteredMills = widget.availableMeals.where((element) => element.categories.contains(category.id)).toList();

    Navigator.push(context, MaterialPageRoute(builder: (ctx) => MealsScreen(title: category.title, meals: filteredMills,)));
  }
  ...
  Widget build(BuildContext context) {
    ...
    CategoryGridItem(category: category,onselecCategory: (){
    _selectCategory(context,category);
    },)
    ],), 
    builder: (context,child) => SlideTransition(position:
      Tween(begin: Offset(0,0.7),end: Offset(0,0),
